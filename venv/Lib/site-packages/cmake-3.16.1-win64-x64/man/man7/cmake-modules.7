...
.
.TH "CMAKE-MODULES" "7" "Dec 10, 2019" "3.16.1" "CMake"
.SH NAME
...
.sp
CMake
can generate \fI\%graphviz\fP files, showing the dependencies between the
targets in a project and also external libraries which are linked
against.  When CMake is run with the \fB\-\-graphviz=foo.dot\fP option, it will
produce:
.INDENT 0.0
.IP \(bu 2
a \fBfoo.dot\fP file showing all dependencies in the project
.IP \(bu 2
a \fBfoo.dot.<target>\fP file for each target, file showing on which other targets the respective target depends
.IP \(bu 2
a \fBfoo.dot.<target>.dependers\fP file, showing which other targets depend on the respective target
.UNINDENT
.sp
The different dependency types \fBPUBLIC\fP, \fBPRIVATE\fP and \fBINTERFACE\fP
are represented as solid, dashed and dotted edges.
.sp
This can result in huge graphs.  Using the file
\fBCMakeGraphVizOptions.cmake\fP the look and content of the generated
graphs can be influenced.  This file is searched first in
\fBCMAKE_BINARY_DIR\fP and then in \fBCMAKE_SOURCE_DIR\fP\&.  If found, it is
...
.SS CPackIFW
.sp
The documentation for the CPack IFW generator has moved here: \fBCPack IFW Generator\fP
.sp
...
.SS Introduction
.sp
The CPack module generates a file \fBCPackConfig.cmake\fP intended for
use in a subsequent run of  the \fBcpack\fP program
where it steers the generation of installers or/and source packages.
.sp
Inclusion of the CPack module adds two new build targets, \fBpackage\fP
and \fBpackage_source\fP, which build the binary and source installers
respectively.  The generated binary installers contain everything
installed via CMake’s \fBinstall()\fP command (and the deprecated
commands \fBinstall_files()\fP, \fBinstall_programs()\fP, and
\fBinstall_targets()\fP).
.sp
For certain kinds of binary installers (including the graphical
installers on macOS and Windows), CPack generates installers that
allow users to select individual application components to install.
See \fBCPackComponent\fP module for further details.
.SS CPack Generators
.sp
...
.sp
The CMake source tree itself contains a \fI\%CPACK_PROJECT_CONFIG_FILE\fP\&.
See the top level file \fBCMakeCPackOptions.cmake.in\fP for an example.
.sp
...
related to them will be ignored.  In most cases, \fB<contentOptions>\fP will
...
\fI\%FetchContent_MakeAvailable()\fP instead (and should be preferred where
...
that pattern is that it will only call \fBadd_subdirectory()\fP on the
...
(As of CMake version 3.16.1 there is only one API version.)
.sp
...
list of libraries searched for is taken from the \fBautoconf\fP macro file,
...
.SS Hints
.sp
Set \fBMKLROOT\fP environment variable to a directory that contains an MKL
installation.
.SS FindBacktrace
.sp
...
other optional components like \fBgtkmm\fP, \fBglade\fP, and \fBglademm\fP\&.
.sp
...
components.  The only valid components are \fBC\fP, \fBCXX\fP, \fBFortran\fP, \fBHL\fP,
and \fBFortran_HL\fP\&.  If the \fBCOMPONENTS\fP argument is not given, the module will
...
.sp
To provide the module with a hint about where to find your \fBHDF5\fP
installation, you can set the environment variable \fBHDF5_ROOT\fP\&.  The
Find module will then look in this path when searching for \fBHDF5\fP
...
.TP
.B \fBBLA_F95\fP
if \fBON\fP tries to find BLAS95/LAPACK95
.UNINDENT
.SS Result Variables
.sp
This module defines the following variables:
...
.SS IMPORTED Targets
.sp
This module defines \fBIMPORTED\fP target \fBLibXml2::LibXml2\fP, if
...
This module defines \fBOPENAL_LIBRARY OPENAL_FOUND\fP, if
false, do not try to link to OpenAL \fBOPENAL_INCLUDE_DIR\fP, where to find
the headers.
.sp
\fB$OPENALDIR\fP is an environment variable that would correspond to the
\fB\&./configure \-\-prefix=$OPENALDIR\fP used in building OpenAL.
.sp
Created by Eric Wing.  This was influenced by the \fBFindSDL.cmake\fP
module.
.SS FindOpenCL
.sp
...
\fBDevelopment\fP and \fBNumPy\fP, specify all components at the same time:
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
.INDENT 0.0
.INDENT 3.5
If components \fBInterpreter\fP and \fBDevelopment\fP are both specified, this
module search only for interpreter with same platform architecture as the one
defined by \fBCMake\fP configuration. This contraint does not apply if only
\fBInterpreter\fP component is specified.
.UNINDENT
.UNINDENT
.SS Imported Targets
.sp
...
\fBDevelopment\fP is found.
.TP
.B \fBPython::Module\fP
Python library for Python module. Target defined if component \fBDevelopment\fP
is found.
.TP
.B \fBPython::NumPy\fP
...
The \fBPython_FIND_STRATEGY\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
...
the \fBPython_FIND_REGISTRY\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
...
\fBCMAKE_FIND_FRAMEWORK\fP variable.
.sp
\fBNOTE:\fP
.INDENT 7.0
.INDENT 3.5
...
only when a virtual environment is active (i.e. the \fBactivate\fP script has
been evaluated). In this case, it takes precedence over
\fBPython_FIND_REGISTRY\fP and \fBCMAKE_FIND_FRAMEWORK\fP variables.
The \fBPython_FIND_VIRTUALENV\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
...
interpreter. In this case, variable \fBPython_FIND_REGISTRY\fP (Windows)
or \fBCMAKE_FIND_FRAMEWORK\fP (macOS) can be set with value \fBLAST\fP or
...
This variable defines how lookup will be done.
The \fBPython2_FIND_STRATEGY\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
\fBVERSION\fP: Try to find the most recent version in all specified
...
of preference between registry and environment variables.
the \fBPython2_FIND_REGISTRY\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
\fBFIRST\fP: Try to use registry before environment variables.
...
only when a virtual environment is active (i.e. the \fBactivate\fP script has
been evaluated). In this case, it takes precedence over
\fBPython2_FIND_REGISTRY\fP and \fBCMAKE_FIND_FRAMEWORK\fP variables.
The \fBPython2_FIND_VIRTUALENV\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
\fBFIRST\fP: The virtual environment is used before any other standard
...
.IP \(bu 2
\fBSTANDARD\fP: The virtual environment is not used to look\-up for the
interpreter. In this case, variable \fBPython2_FIND_REGISTRY\fP (Windows)
or \fBCMAKE_FIND_FRAMEWORK\fP (macOS) can be set with value \fBLAST\fP or
\fBNEVER\fP to select preferably the interpreter from the virtual
environment.
.UNINDENT
...
This variable defines how lookup will be done.
The \fBPython3_FIND_STRATEGY\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
\fBVERSION\fP: Try to find the most recent version in all specified
...
of preference between registry and environment variables.
The \fBPython3_FIND_REGISTRY\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
\fBFIRST\fP: Try to use registry before environment variables.
...
only when a virtual environment is active (i.e. the \fBactivate\fP script has
been evaluated). In this case, it takes precedence over
\fBPython3_FIND_REGISTRY\fP and \fBCMAKE_FIND_FRAMEWORK\fP variables.
The \fBPython3_FIND_VIRTUALENV\fP variable can be set to empty or one of the
following:
.INDENT 7.0
.IP \(bu 2
\fBFIRST\fP: The virtual environment is used before any other standard
...
.IP \(bu 2
\fBSTANDARD\fP: The virtual environment is not used to look\-up for the
interpreter. In this case, variable \fBPython3_FIND_REGISTRY\fP (Windows)
or \fBCMAKE_FIND_FRAMEWORK\fP (macOS) can be set with value \fBLAST\fP or
\fBNEVER\fP to select preferably the interpreter from the virtual
environment.
.UNINDENT
...
standard syntax, e.g.   \fBfind_package(SWIG 1.1)\fP
.sp
...
Instead, list \fBCUDA\fP among the languages named in the top\-level
call to the \fBproject()\fP command, or call the
